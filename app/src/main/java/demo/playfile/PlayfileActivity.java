...
public class PlayfileActivity extends Activity implements OnItemClickListener {
    ...
    class SearchTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... params) {
            ...
            try {
                NtlmPasswordAuthentication mAuthentication = new NtlmPasswordAuthentication("", "PC-001", "jem");
                SmbFile smbFile = new SmbFile("smb://192.168.33.5/", mAuthentication);
                ...
                for (SmbFile f : fs) {
                    if (f.getName().contains("Program")) {
                        SmbFile[] smbFiles = f.listFiles();
                        List<SmbFile> fileList1 = Arrays.asList(smbFiles);
